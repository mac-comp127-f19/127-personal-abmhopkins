...
public class WebWordCounter {
    ...
    public static void main(String [] args) {
        ...
        List<String> testWords = urlToWords(TEST_URL);
        ...
        for(String word : testWords){
            if (!isStopWord(word)) {
                if (wCounter.containsKey(word)) {
                    temp = wCounter.get(word);
                    temp = temp + 1;
                    wCounter.put(word, temp);
                } else {
                    wCounter.put(word, 1);
                }
            }
        ...
        for(String aWord: testWords) {
            if(!isStopWord(aWord)) {
                if(wCounter.containsKey(aWord)) {
                    temp = wCounter.get(aWord);
                    wCounter.put(aWord, ++temp);
                }
                else {
                    wCounter.put(aWord, 1);
                }
            }
        }
//to display the entries that have value of 50 or more
        System.out.println(wCounter.entrySet().stream()
