...
public class Emojis {
    ...
    public static GraphicsGroup createSmileyFace(double size) {
        ...
        Ellipse leftEye = createEye(size*0.12);
        leftEye.setCenter(size*0.33, size*0.33);
        group.add(leftEye);
        Ellipse rightEye = createEye(size*0.12);
        rightEye.setCenter(size*0.67, size*0.33);
        group.add(rightEye);
    ...
    public static GraphicsGroup createWinkyFace(double size) {
        ...
        leftEye.setCenter(size*0.33, size*0.33);
        group.add(leftEye);
        Rectangle rightEye = createWink(size*0.15,size*0.025);
        rightEye.setCenter(size*0.67,size*0.33);
        group.add(rightEye);
    ...
    /**
     * Creates one eye. The size of the eye is relative to the parameter size.
     */
    ...
    private static Rectangle createWink(double width, double height) {
        Rectangle eye = new Rectangle(0,0, width, height);
        eye.setFillColor(EYE_COLOR);
        return eye;
    }
    /**
     * Creates a frown-shaped arc. The arc is measured relative to its “implied ellipse,” which is
     * the shape that would be formed if the arc were extend all the way around. The size of the
     * resulting arc will be smaller than the implied ellipse’s size.
     *
     * @param ellipseWidth  The width of the implied ellipse from which the smile’s arc is cut.
